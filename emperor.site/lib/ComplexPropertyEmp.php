...
class ComplexPropertyEmp
{
    ...
    public static function GetUserTypeDescription()
    {
        return array(
            'PROPERTY_TYPE' => 'S',
            'USER_TYPE' => 'C',
            'DESCRIPTION' => 'Комплексное и рабочее свойство',
    ...
    if(!empty($arProperty['USER_TYPE_SETTINGS'])){
        ...
        return '<span>'.'Не заполнен список полей'.'</span>';
    }
...
private static function showHtmlEditor($code, $title, $arValue, $strHTMLControlName)
{
    ...
    if(CModule::IncludeModule('fileman'))
    {
        ...
        $editorName = str_replace(['[', ']'], '_', $strHTMLControlName['VALUE'].'['.$code.']');
        $editorName = preg_replace('/_+/', '_', $editorName);
        
        $editorId = 'html_editor_'.$editorName;
        
        CFileMan::AddHTMLEditorFrame(
            $editorId,
            $v,
            $editorId."_TYPE",
            strip_tags($v) !== $v ? "html" : "text",
            array(
                'height' => 300,
                'width' => '100%',
        ...
        $result .= '<tr>
                    ...
                    <td>
                        <div style="max-width: 800px;">'.$editorContent.'</div>
                        <input type="hidden" name="'.$strHTMLControlName['VALUE'].'['.$code.']" id="'.$editorId.'_hidden" value="'.htmlspecialcharsbx($v).'">
                    </td>
                </tr>';
        ...
        <script>
            BX.ready(function(){
                var editor = BX("'.$editorId.'");
                var hiddenInput = BX("'.$editorId.'_hidden");
                
                if(editor && hiddenInput) {
                    BX.bind(editor, "keyup", function() {
                        hiddenInput.value = editor.value;
                    });
                    BX.bind(editor, "change", function() {
                        hiddenInput.value = editor.value;
                    });
                }
            });
    ...
    else
    {
        $result .= '<tr>
                    <td align="right" valign="top">'.$title.': </td>
                    <td><textarea rows="12" name="'.$strHTMLControlName['VALUE'].'['.$code.']" style="width:100%; min-height:300px;">'.htmlspecialcharsbx($v).'</textarea></td>
                </tr>';
    }
    ...
    if(!empty($value['VALUE'])){
        ...
        foreach ($data as $code => $val){
            $title = $arFields[$code]['TITLE'] ?? $code;
            $type = $arFields[$code]['TYPE'] ?? '';
            
            if($type === 'html'){
                $result .= '<div><strong>'.$title.':</strong><div>'.$val.'</div></div>';
            }
            else{
                $result .= '<div><strong>'.$title.':</strong> '.$val.'</div>';
            }
        }
    }
            ...
            foreach ($arSetting as $code => $arItem) {
                $result .= '
                       <tr valign="top">
                           ...
                           <td><input type="text" class="inp-title" size="35" name="'.$strHTMLControlName["NAME"].'['.$code.'][TITLE]" value="'.$arItem['TITLE'].'"></td>
                           <td><input type="text" class="inp-sort" size="5" name="'.$strHTMLControlName["NAME"].'['.$code.'][SORT]" value="'.$arItem['SORT'].'"></td>
                           <td>
                                <select class="inp-type" name="'.$strHTMLControlName["NAME"].'['.$code.'][TYPE]">
    ...
    public static function PrepareUserSettings($arProperty)
    {
          $result = [];
         if(!empty($arProperty['USER_TYPE_SETTINGS'])){
            $result = $arProperty['USER_TYPE_SETTINGS'];
         }
        return $result;
    }
            ...
            function run() {
                ...
                jQuery(document).on('change', '.inp-code', function () {
                    ...
                    if (code.length <= 0) {
                        ...
                        row.find('input.inp-title').attr('name', '<?=$inputName?>[' + code + '][TITLE]');
                        row.find('input.inp-sort').attr('name', '<?=$inputName?>[' + code + '][SORT]');
                        row.find('select.inp-type').attr('name', '<?=$inputName?>[' + code + '][TYPE]');
                    }
                });
...
private static function prepareSetting($arSetting)
{
    ...
    foreach ($arSetting as $code => $arItem) {
        if(is_array($arItem) && isset($arItem['TITLE']) && isset($arItem['TYPE'])) {
            $arResult[$code] = [
                'TITLE' => $arItem['TITLE'],
